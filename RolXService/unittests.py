...
class APITestCase(unittest.TestCase):
    ...
    def test_invalid_json(self):
        ...
        response = requests.get(self.BASE_URL, headers=headers, data='not a json')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json(), {'message': 'Invalid input, JSON required like: \{"query":"SELECT * FROM data LIMIT 5"\}'})

    def test_missing_query(self):
        headers = {'Authorization': self.RETOS_API_TOKEN}
        response = requests.get(self.BASE_URL, headers=headers, json={})
        self.assertEqual(response.status_code, 400)
    ...
    def test_successful_query(self):
        ...
        response = requests.get(self.BASE_URL, headers=headers, json=query)
